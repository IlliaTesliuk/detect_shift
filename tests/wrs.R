...
wrs_test = function(x1,y1,x2,y2,type="KLR"){
  
  n1 = nrow(x1)
  n2 = nrow(x2)
  
  n12 <- ceiling(n1*0.5); n22 <- ceiling(n2*0.5)
  ...
  if(type=="LR"){
    ...
    xy.fit = data.frame(poly(xy.fit, degree = 2, raw = T))
    fit.joint = glm(label.fit~., data=xy.fit, family="binomial")
    x.fit <- rbind(x11,x21)
    x.fit = data.frame(poly(x.fit, degree = 2, raw = T))
    fit.marginal <- glm(label.fit~., data=x.fit, family="binomial")
    newdata = data.frame(poly(rbind(x12,x22),degree = 2,raw = T))
    prob.marginal <- predict(fit.marginal, newdata=newdata, type="response")
    prob.marginal[prob.marginal<0.01] <- 0.01; prob.marginal[prob.marginal>0.99] <- 0.99
    g12.est.ql <- prob.marginal[1:n12]/(1-prob.marginal[1:n12])*n11/n21
    g22.est.ql <- prob.marginal[(n12+1):(n12+n22)]/(1-prob.marginal[(n12+1):(n12+n22)])*n11/n21
    newdata = data.frame(poly(cbind(rbind(x12,x22), c(y12,y22)), degree = 2, raw = T))
    prob.joint <- predict(fit.joint, newdata=newdata, type="response")
    prob.joint[prob.joint<0.01] <- 0.01; prob.joint[prob.joint>0.99] <- 0.99
    v12.est.ql <- (1-prob.joint[1:n12])/prob.joint[1:n12]*g12.est.ql
    v22.est.ql <- (1-prob.joint[(n12+1):(n12+n22)])/prob.joint[(n12+1):(n12+n22)]*g22.est.ql
    temp <- getFinalStat(g12.est.ql, g22.est.ql, v12.est.ql, v22.est.ql)
  }else if(type=="KLR"){
    label.fit <- as.factor(c(rep(0,n11), rep(1,n21)))
    xy.fit <- cbind(rbind(x11, x21), c(y11, y21))
...
wrs_test2 = function(x1,y1,x2,y2,type="KLR"){
